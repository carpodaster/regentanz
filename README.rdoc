...
If you want to use the latest version, use GitHub:
 # Gemfile
...
In order to handle errors, a certain mailer is needed. The gem expects
 SupportMailer#weather_retry_marker_notification

The mailer-method gets two arguments: an instance of +Regentanz::GoogleWeather+ and
a symbol informing wether the API-failure just happened (+:set+) or has been remedied (+:unset+).
